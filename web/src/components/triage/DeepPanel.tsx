...
// for posting onto the issue itself.
export function formatReportComment(r: DeepReport): string {
  ...
  if (r.relatedIssues?.length) {
    lines.push("");
    lines.push("**Related issues:** " + r.relatedIssues.map((ri) => `${ri.identifier} (${ri.state})`).join(" · "));
  }
  lines.push("");
              ...
              <ul className="mt-1.5 space-y-1">
                {report.evidence.map((e, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs text-muted-foreground">
                    <span className={cn("mt-px shrink-0 font-mono text-[10px] font-bold uppercase", SOURCE_TONE[e.source] ?? "")}>
                      {e.source}
                    </span>
                    <span className="min-w-0">
                      {e.finding}
            ...
            <div>
              ...
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {report.relatedIssues.map((ri, i) => (
                  <span key={i} className="rounded-full border border-border bg-surface-2 px-2.5 py-1 text-[11px] text-muted-foreground" title={ri.title}>
